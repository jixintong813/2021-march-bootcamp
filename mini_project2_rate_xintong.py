...
def get_rate(client_id):
    ...
    print(response)
    return round(float(response.content),1)
    # Sample end
# -- TODO END: Part 2
...
def test_upsert_rate():
    upsert_client_rate('client8',0.8)
    upsert_client_rate('client9',0.6)
    assert get_rate('client8') == 0.8
    assert get_rate('client9') == 0.6
# -- TODO END: Part 6
